...
class TimeSformer(nn.Module):
    def __init__(
        self, img_size=224, patch_size=16, num_classes=400, num_frames=8, attention_type="divided_space_time", pretrained_model="", **kwargs
    ):
...
class FlowTokenVisionTransformer(VisionTransformer):
    def __init__(self, *args, img_size=224, patch_size=16, in_chans=3, embed_dim=768, **kwargs):
        super(FlowTokenVisionTransformer, self).__init__(
            *args, img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim, **kwargs
        )
        ...
        self.flow_patch_embed = PatchEmbed(img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim)
        for blk in self.blocks:
            blk.class_tokens = 2
...
class AuxTokenVisionTransformer(VisionTransformer):
    def __init__(self, *args, img_size=224, patch_size=16, in_chans=3, embed_dim=768, **kwargs):
        super(AuxTokenVisionTransformer, self).__init__(
            *args, img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim, **kwargs
        )
        self.aux_cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.n_cls_tokens = 2
...
def get_vit_base_patch16_224(no_head=False, **kwargs):
    patch_size = 16
    ...
    vit.num_patches = (224 // patch_size) * (224 // patch_size)
    pretrained_model = ""
    if pretrained_model:
        load_pretrained(
            vit,
            ...
            attention_type=vit.attention_type,
            pretrained_model=pretrained_model,
        )
    if no_head:
        vit.head = None

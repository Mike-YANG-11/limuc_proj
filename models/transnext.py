...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W, relative_pos_index, relative_coords_table):
        ...
        attn = ((F.normalize(q, dim=-1) + self.query_embedding) * F.softplus(self.temperature) * self.seq_length_scale) @ F.normalize(
            k, dim=-1
        ).transpose(-2, -1) + rel_bias
    ...
    def __init__(
        ...
        for i in range(num_stages):
            ...
            relative_pos_index, relative_coords_table = get_relative_position_cpb(
                ...
                key_size=(
                    to_2tuple(img_size // ((2 ** (i + 2)) * sr_ratios[i]))
                    if (fixed_pool_size is None or sr_ratios[i] == 1)
                    else to_2tuple(fixed_pool_size)
                ),

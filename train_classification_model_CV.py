...
def get_test_set_results(id, test_dir, normalize):
    ...
    y_true, y_probs, y_pred, r_true, r_probs, r_pred = get_test_results_classification(model, test_loader, device, True, [2, 3])
...
def run_experiment(experiment_id: int, train_dir: str, val_dir: str, normalize, best_acc=0, early_stop_counter=0):
    ...
    experiment_signature = (
        model_name + " ID=" + str(experiment_id) + " lr=" + str(learning_rate) + " reg=" + str(weight_decay) + " bs=" + str(batch_size)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--CV_fold_path", type=str, required=True, help="location of train-val folds.")
    ...
    class_precisions = np.zeros([number_of_experiments, num_classes])
    class_recalls = np.zeros([number_of_experiments, num_classes])
    class_f1s = np.zeros([number_of_experiments, num_classes])
    ...
    for i in range(number_of_experiments):
        ...
        if enable_wandb:
            wandb.init(project="ulcerative-colitis-classification", group=model_name + "_CV_C" + "_" + group_id, save_code=True, reinit=True)
            wandb.run.name = model_name + "_cls_epoch_" + str(id)
            wandb.run.save()
        ...
        y_true, y_probs, y_pred, r_true, r_probs, r_pred = get_test_set_results(id, test_dir, normalize)
